...
export default function Designation() {
  ...
  const [editDesignationData, setEditDesignation] = useState(false);
  ...
  const handleDelete=(id)=>{
    setLoading(true)
    Api.delete(`/designations/${id}`).then((res)=>{
      if(res.data.success){
        getData()
      }
     })
  ...
  return (
    <>
      ...
      <Container>
        {addDesignation && <CModel filter="add-designation" width='40%' open={addDesignation} setOpen={setAddDesignation}  recall={getData} />}
        {editDesignationData && <CModel filter="edit-designation" width='40%' open={editDesignationData} setOpen={setEditDesignation} data={editData} recall={getData} />}
       <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
            Designations Management
          </Typography>
          <Button onClick={handleAddDesignation} variant="contained" startIcon={<Iconify icon="eva:plus-fill" />}>
            New Designation
          </Button>
        </Stack>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,index) => {
                    const { id, designation_name,  status,} = row;
                    const selectedUser = selected.indexOf(designation_name) !== -1;
